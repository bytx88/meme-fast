...
$('save-token').addEventListener('click',()=>{const token=viewData().tokens[0]||state.token;if(state.tokens.length!==1)return;toggleSaved({type:'token',id:listingKey(token),title:`$${token.symbol} · ${token.name}`,subtitle:`${token.network} · ${short(token.address)}`,href:`./order-flow.html?query=${encodeURIComponent(token.address)}`});updateIdentity()});
