...
export async function loadListings(input,request,progress=()=>{},isCurrent=()=>true){
  ...
  for(const token of tokens){
    ...
    try{
      ...
      for(const p of unique){const key=`${token.network}:${canonical(p.attributes.address)}`;if(!poolsByKey.has(key))poolsByKey.set(key,{key,address:p.attributes.address,name:p.attributes.name,network:token.network,dex:p.relationships?.dex?.data?.id||'',count:0,failed:false,targets:[]});poolsByKey.get(key).targets.push(token)}
  ...
  for(const pool of pools){
    ...
    try{const result=await request(`/networks/${encodeURIComponent(pool.network)}/pools/${encodeURIComponent(pool.address)}/trades`);pool.count=result.data.length;
    ...
    }catch(e){pool.failed=true;pool.error=e.message}
  }
