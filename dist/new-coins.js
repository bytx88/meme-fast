...
import {stageFor} from './coin-stages.mjs?v=post-migration-v1';
...
function lifecycleTag(c){return c.ruggedAt?'<span class="lifecycle-tag rugged" title="Observed price fell at least 78% from a post-migration peak. This does not establish fraud.">Rugged</span>':c.sustainedAt?'<span class="lifecycle-tag sustained">Sustained</span>':''}
...
function render(){
 ...
 const all=candidates(),rows=groupCoins(all,context),stages={new:[],stretch:[],sustained:[],migrated:[]};
 ...
 visibleRows=state.view==='discover'?[...stages.stretch,...stages.new,...stages.sustained,...stages.migrated]:rows;
