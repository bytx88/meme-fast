...
export function tokenActions(coin){
 ...
 return `<span class="token-actions compact-token-actions"><button type="button" class="ca-copy" data-copy-ca="${esc(coin.id)}" title="${esc(address)}" aria-label="Copy ${esc(coin.symbol)} contract address">CA ⧉</button><a class="token-detail" href="./?query=${encodeURIComponent(address)}" aria-label="Open ${esc(coin.symbol)} token research">Detail ↗</a>${axiom?`<a class="axiom-link" href="${esc(axiom)}" target="_blank" rel="noopener noreferrer" aria-label="Open ${esc(coin.symbol)} on Axiom">Axiom ↗</a>`:''}</span>`;
}
